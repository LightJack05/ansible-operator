...
# lightjack.de/ansible-operator-bundle:$VERSION and lightjack.de/ansible-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= lightjack.de/ansible-operator
...
IMG ?= ghcr.io/lightjack05/ansible-operator
...
KIND_CLUSTER ?= ansible-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name ansible-operator-builder
	$(CONTAINER_TOOL) buildx use ansible-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm ansible-operator-builder
	rm Dockerfile.cross
