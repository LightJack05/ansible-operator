...
var _ = Describe("AnsibleHost", func() {
	AfterEach(dumpControllerLogsOnFailure)
	...
	It("Should become ready and create a secret when configured correctly", func() {
		...
		createSSHHosts(testNs)

		By("Creating a valid AnsibleHost resource")
		createSSHKeySecret(testNs, "ssh-node-0-credentials")

		createValidAnsibleHost(
			"valid-ansible-host",
			testNs,
			fmt.Sprintf("ssh-node-%d.%s.svc.cluster.local", 0, testNs),
			"root",
	...
	It("Should become not ready when configured incorrectly, and not create a secret", func() {
		...
		By("Creating an AnsibleHost resource pointing at a nonexistent host")
		createSSHKeySecret(testNs, "ssh-node-0-credentials")

		createValidAnsibleHost(
			"ansible-host",
			testNs,
			fmt.Sprintf("invalid-ssh-node-%d.%s.svc.cluster.local", 0, testNs),
			"root",
			"ssh-node-0-credentials",
			"ssh-node-0-hostkey",
			22,
	...
	It("Should fail when there is no private key secret", func() {
		...
		By("Creating an AnsibleHost resource without its credentials secret")
	...
	It("Should fail when the private key secret is bogus", func() {
		...
		By("Creating an AnsibleHost resource with a bogus credentials secret")
